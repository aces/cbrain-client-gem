...
module CbrainClient
  class TasksApi
    ...
    # @param tasklist The IDs or batch IDs of the tasks
    # @param operation The operation to perform on the selected tasks
    # @param [Hash] opts the optional parameters
    # @return [Object]
    def tasks_operation_post(tasklist, operation, opts = {})
      data, _status_code, _headers = tasks_operation_post_with_http_info(tasklist, operation, opts)
      data
    end
    ...
    def tasks_operation_post_with_http_info(tasklist, operation, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: TasksApi.tasks_operation_post ...'
      end
      # verify the required parameter 'tasklist' is set
      if @api_client.config.client_side_validation && tasklist.nil?
        fail ArgumentError, "Missing the required parameter 'tasklist' when calling TasksApi.tasks_operation_post"
      end
      # verify the required parameter 'operation' is set
      if @api_client.config.client_side_validation && operation.nil?
        fail ArgumentError, "Missing the required parameter 'operation' when calling TasksApi.tasks_operation_post"
      end
      # verify enum value
      if @api_client.config.client_side_validation && !['delete', 'terminate', 'archive', 'unarchive', 'zap_wd', 'recover', 'restart_setup', 'restart_cluster', 'restart_postprocess'].include?(operation)
        fail ArgumentError, "invalid value for 'operation', must be one of delete, terminate, archive, unarchive, zap_wd, recover, restart_setup, restart_cluster, restart_postprocess"
      end
      # resource path
      local_var_path = '/tasks/operation'
      ...
      # http body (model)
      post_body = @api_client.object_to_http_body(tasklist)
      auth_names = ['BrainPortalSession']
      data, status_code, headers = @api_client.call_api(:POST, local_var_path,
        ...
        :return_type => 'Object')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: TasksApi#tasks_operation_post\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
    # Create a new Task.
